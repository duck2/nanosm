...
async def write_reg(dut, reg, values, mask=0xFF):
...
async def read_reg(dut, reg, port='a'):
    """Read all lanes from a register on specified port (1 cycle latency)."""
    getattr(dut, f"rd_addr_{port}").value = reg
    await RisingEdge(dut.clk)
    await Timer(1, unit='ps')
    return [getattr(dut, f"rd_data_{port}[{t}]").value.integer for t in range(LANES)]
...
async def test_rf(dut):
    ...
    # Basic read/write
    test_vals = [0x1000 + t for t in range(LANES)]
    ...
    # Write mask test
    initial_vals = [0x5000 + t for t in range(LANES)]
    new_vals = [0x6000 + t for t in range(LANES)]
    await write_reg(dut, 5, initial_vals)
    await write_reg(dut, 5, new_vals, mask=0xAA)  # Only odd threads
    await RisingEdge(dut.clk)
    ...
    # R0 always zero
    await write_reg(dut, 0, [0xDEADBEEF] * LANES)
    await RisingEdge(dut.clk)
    ...
    for reg in range(1, 32):
        vals = [random.randint(0, 0xFFFFFFFF) for _ in range(LANES)]
